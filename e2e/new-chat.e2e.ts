...
test("empty chats render no hover tip", async ({ page }) => {
	...
	await expect(
		page.locator("aside:not(.settings-panel) .side-tip")
	).toHaveCount(1);
});

test("row tip answers the row button, not its action buttons", async ({
	page
}) => {
	await seedChat(page, [{ role: "user", content: "hi" }]);
	await page.goto("/");
	...
	await openSidebar(page);
	const row = page
		.locator("aside:not(.settings-panel) li:has(.side-tip)")
		.first();
