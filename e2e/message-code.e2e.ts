...
test.beforeEach(async ({ page }) => {
	...
	await page.goto("/");
	await expect(page.locator(".ccez-code").first()).toBeVisible({
		timeout: 60_000
	});
	// Shiki enhancement is async: wait for highlighted spans.
	await expect(page.locator(".ccez-code .shiki span").first()).toBeVisible({
		timeout: 20_000
	});
});
...
test("assistant python block is highlighted and narrow", async ({ page }) => {
	expect(await page.locator(".ccez-code .shiki span").count()).toBeGreaterThan(
		0
	);
	const sizes = await page.evaluate(() => {
		...
		return {
			...
			column: block.parentElement?.offsetWidth ?? 0
		};
	});
...
test("dark code stays contrasted", async ({ page }) => {
	...
	const ratio = await page.evaluate(() => {
		...
		const contrast = (fg: string, bg: string): number => {
			const [a, z] = [lum(fg), lum(bg)].sort((x, y) => y - x) as [
				number,
				number
			];
			return (a + 0.05) / (z + 0.05);
		};
		const cs = (el: Element, p: string): string =>
		...
		const tokens = Array.from(
			document.querySelectorAll(".ccez-code .shiki span")
		) as HTMLElement[];
		const preBg = cs(pre, "background-color");
		const worst = Math.min(
			...tokens.map((t) => contrast(cs(t, "color"), preBg))
		);
		return {
...
test("assistant code block has a copy button with no fold bar", async ({
	page
}) => {
...
test("right-click toggles the code fold, left-click unfolds, staying silent", async ({
	page
}) => {
	const block = page.locator(".ccez-code").first();
	const pre = block.locator("pre");
