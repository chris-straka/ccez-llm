 ...
 */
async function mockViewport(page: Page, height: number, offsetTop: number): Promise<void> {
	await page.evaluate(([h, o]: [number, number]) => {
...
test("fallback pin engages on no-shrink open and releases on close", async ({ page }) => {
	await seedEmpty(page);
	// Layout full (real 915), visual 615: 300px of keyboard, no pan.
	await mockViewport(page, 615, 0);
	await mockViewport(page, 615, 0);
	await expect.poll(async () => (await appPin(page)).h, { timeout: 5000 }).toBe("615px");
	expect((await appPin(page)).kb).toBe("300px");
	// Keyboard gone: the pin and the reflow var clear.
	await mockViewport(page, 915, 0);
	await expect.poll(async () => (await appPin(page)).h, { timeout: 5000 }).toBe("");
