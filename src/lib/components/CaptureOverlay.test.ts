 ...
 * Enter sends (never mid-IME-composition), Esc stays on the page
 * ladder (which also returns focus). Layout is invisible to jsdom, so
 * this asserts on source like the other component contracts.
 */
...
describe("capture overlay", () => {
	...
	it("sends on Enter but never mid-composition", () => {
		const source = overlaySource();
	...
	it("exposes send and cancel with no Esc of its own", () => {
		const source = overlaySource();
