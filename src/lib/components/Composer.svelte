...
<script lang="ts">
	...
	const openAnnotation = $derived(
		highlightId ? (annotations.find((a) => a.id === highlightId) ?? null) : null
	);
	// The open row joins only with the review open (badge tap):
	// answer-card opens keep their highlight without sprouting
	// a pill.
...
>
	<div class="prompt-tools">
		...
		{#if dockItems.length > 0}
			<!-- Pinned-annotations dock through ReviewDock: the page
			keeps the array, ids, and behaviors; the component owns
			the dock and its surfaces (no inline editor: the pencil
			opens the edit card, pin approval lives on the badge). -->
			<ReviewDock
				items={dockItems}
				open={reviewOpen}
				bind:highlightId
...
<style>
	...
	size (⌘[ / ⌘], the slider — never the message text size), so a
	fixed 0.95rem box next to huge prompt type never strands the
	eyes. Phones keep the 16px floor below: iOS zooms smaller
	fields on focus. */
	...
	.prompt {
		/* Own size track (--prompt-font, set on .app from the prompt
		text-size setting): every --font-scale read inside the
		composer — the typed text above all — follows the prompt's
		zoom, never the message text size. The fallback keeps
		standalone renders (tests, previews) at 1. */
		--font-scale: var(--prompt-font, 1);
