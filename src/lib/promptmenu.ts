...
/**
 * Composer-only native OS text menu.
 *
 ...
 * the "inside the composer" fact (CodeMirror renders the prompt, so
...
/** Pure read: is there a live range anchored inside the composer? */
export function composerSelectionActive(
	promptEl: ContainsNode | null | undefined,
	anchor: Node | null,
	collapsed: boolean
): boolean {
	if (promptEl == null || anchor == null || collapsed) return false;
	try {
		return promptEl.contains(anchor);
	} catch {
		return false;
	}
}
...
/**
 * Report the current composer-selection state; invokes only on
 * transitions so handle drags don't chatter the bridge. The sink is
 * injectable for tests; production passes none.
 */
export function reportPromptMenu(
	promptEl: ContainsNode | null | undefined,
	anchor: Node | null,
	collapsed: boolean,
	sink: (allowed: boolean) => unknown = defaultSink
): void {
	const active = composerSelectionActive(promptEl, anchor, collapsed);
...
export function resetPromptMenuLatch(): void {
	lastReported = false;
}
