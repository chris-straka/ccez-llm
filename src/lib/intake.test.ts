...
import { dropFilesFromDataTransfer, isPermissionDismissal } from "./intake";
...
describe("dropFilesFromDataTransfer", () => {
	...
	it("treats null, missing, and null entries as no files", () => {
		...
		expect(
			dropFilesFromDataTransfer({ files: [null, textFile("a.md")] })
		).toHaveLength(1);
	});
});
describe("isPermissionDismissal", () => {
	it("swallows Abort and NotAllowed, surfaces the rest", () => {
		expect(isPermissionDismissal(new DOMException("x", "AbortError"))).toBe(
			true
		);
		expect(
			isPermissionDismissal(new DOMException("x", "NotAllowedError"))
		).toBe(true);
		expect(
			isPermissionDismissal(new Error("Couldn't capture that frame."))
		).toBe(false);
